 ...
 * @description FIXED: Standardized for role 'CFO' with 18 Accounting Modules.
 */
...
const Layout = (function() {
  ...
  let _state = {
    currentUser: null,
    activeRole: 'CFO', // ✅ تم التوحيد ليطابق الـ HTML الخاص بكِ
    isInitialized: false,
    sidebarOpen: false,
    notifications: [],
    unreadCount: 0
  ...
  const _translations = {
    ar: {
      financial_control: 'الرقابة المالية', general_ledger: 'الأستاذ العام', accounts_payable: 'الحسابات الدائنة (AP)', accounts_receivable: 'الحسابات المدينة (AR)', inventory_assets: 'المخزون والأصول', reports_tax: 'التقارير والضرائب', settings_personal: 'الإعدادات والحساب', dashboard: 'نظرة عامة', approvals: 'الاعتمادات', internal_chat: 'المراسلات الداخلية', gl_journal: 'قيود اليومية', gl_coa: 'دليل الحسابات', gl_cost_centers: 'مراكز التكلفة', ap_bills: 'الفواتير الواردة', ap_payments: 'أوامر الصرف', ap_vendors: 'الموردين', ar_invoices: 'فواتير المبيعات', ar_receipts: 'سندات القبض', inv_dashboard: 'لوحة المخزون', inv_assets: 'سجل الأصول الثابتة', rep_statements: 'القوائم المالية', rep_budget: 'الموازنة التقديرية', rep_tax: 'الإقرارات الضريبية', fin_settings: 'الإعدادات المالية', my_profile: 'الملف الشخصي', notifications: 'التنبيهات المالية', markAllRead: 'تعليم الكل كمقروء', logout: 'تسجيل الخروج', logoutConfirm: 'هل أنت متأكد؟', poweredBy: 'تطوير', aymanDev: 'أيمن المغربي'
    },
    en: {
      financial_control: 'Financial Control', general_ledger: 'General Ledger', accounts_payable: 'Accounts Payable', accounts_receivable: 'Accounts Receivable', inventory_assets: 'Inventory & Assets', reports_tax: 'Reports & Tax', settings_personal: 'Settings', dashboard: 'Overview', approvals: 'Approvals', internal_chat: 'Internal Chat', gl_journal: 'Journal Entries', gl_coa: 'Chart of Accounts', gl_cost_centers: 'Cost Centers', ap_bills: 'Vendor Bills', ap_payments: 'Payments', ap_vendors: 'Vendors', ar_invoices: 'Sales Invoices', ar_receipts: 'Receipts', inv_dashboard: 'Inventory Dash', inv_assets: 'Fixed Assets', rep_statements: 'Financial Statements', rep_budget: 'Budgeting', rep_tax: 'Tax Reports', fin_settings: 'Finance Settings', my_profile: 'My Profile', notifications: 'Finance Alerts', markAllRead: 'Mark all as read', logout: 'Logout', logoutConfirm: 'Are you sure?', poweredBy: 'Developed by', aymanDev: 'Ayman Almaghrabi'
    }
  };
  ...
  function loadNotifications() {
    _state.notifications = [
      { id: 'F1', icon: 'fa-file-invoice-dollar', color: 'red', title: { ar: 'فاتورة معلقة', en: 'Pending Bill' }, body: { ar: 'بانتظار الاعتماد المالي', en: 'Awaiting financial approval' }, time: new Date(), read: false, link: 'approvals.html' }
    ];
    _state.unreadCount = _state.notifications.filter(n => !n.read).length;
  }

  // ==========================================
  // 5. RENDER SIDEBAR (Fixed logic)
  // ==========================================
  function renderSidebar() {
    ...
    // ✅ التعديل الجذري: نستخدم الدور CFO مباشرة لضمان ظهور القائمة
    ...
    container.innerHTML = `
      <aside id="main-sidebar" class="fixed top-0 ${isRTL ? 'right-0 border-l' : 'left-0 border-r'} z-50 h-screen w-72 flex flex-col bg-white dark:bg-[#0F172A] border-slate-200 dark:border-slate-800 shadow-2xl transition-all duration-300">
        ...
        <div class="p-4">
          <div class="flex items-center gap-3 p-3 rounded-xl bg-slate-50 dark:bg-slate-800 border dark:border-slate-700">
            ...
            <div class="min-w-0">
              ...
              <p class="text-[9px] text-brandRed font-bold uppercase">CFO / Manager</p>
            </div>
          </div>
        </div>
        ...
        <div class="p-4 text-center border-t border-slate-100 dark:border-slate-800 bg-slate-50/50">
          <p class="text-[10px] text-slate-400 font-medium">© 2026 ERP Finance</p>
      ...
      <header class="h-20 sticky top-0 z-40 flex items-center justify-between px-6 bg-white/90 dark:bg-[#0F172A]/95 backdrop-blur-xl border-b border-slate-200 dark:border-slate-800">
        <div class="flex items-center gap-4">
          ...
          <div class="px-4 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-[11px] font-bold text-slate-600 dark:text-slate-300">
             <i class="fa-solid fa-lock text-brandRed mr-2"></i> SECURE_FINANCE_v10.5
          </div>
        </div>
        <div class="flex items-center gap-3">
          ...
          <button onclick="if(window.AndroBot) AndroBot.toggle()" class="w-10 h-10 rounded-xl border-2 border-slate-200 dark:border-slate-700 text-brandBlue flex items-center justify-center group"><i class="fa-solid fa-robot"></i></button>
          ...
          <button onclick="Layout.logout()" class="text-red-500 hover:bg-red-50 px-4 py-2 rounded-xl text-xs font-bold transition-all"><i class="fa-solid fa-power-off"></i> ${t('logout')}</button>
        </div>
      </header>`;
  }
