/**
 * Shareholder Portal Layout v2.0
 * تم التحديث: إزالة مؤشرات السوق + إضافة خدمات التحديث والطلبات
 */
(function() {
    ...
    function renderSidebar() {
        ...
        const menu = [
            ...
            { link: "profile.html", icon: "fa-id-card", text: "بياناتي والحساب البنكي" }, // جديد
            { link: "requests.html", icon: "fa-headset", text: "الطلبات والتواصل" },     // جديد
            ...
            { link: "voting.html", icon: "fa-check-to-slot", text: "الجمعيات والتصويت" }
        ];
        ...
        let links = menu.map(m => `
            <a href="${m.link}" class="flex items-center gap-3 px-4 py-3 text-slate-400 hover:text-white hover:bg-white/10 rounded-xl transition-all mb-1 ${window.location.href.includes(m.link)?'bg-brandRed text-white':''}">
                <i class="fa-solid ${m.icon} w-6 text-center"></i> <span class="font-bold text-sm">${m.text}</span>
            </a>
        `).join('');
        ...
        <aside class="fixed top-0 ${isRtl ? 'right-0' : 'left-0'} h-screen w-72 bg-slate-900 text-white flex flex-col z-50">
            <div class="h-20 flex items-center px-8 border-b border-slate-800">
                <h1 class="text-xl font-extrabold tracking-tight">Andro<span class="text-brandRed">Gov</span> <span class="text-[10px] bg-slate-800 px-2 py-1 rounded text-slate-400">IR</span></h1>
            </div>
            <div class="p-6">
                <div class="bg-slate-800/50 rounded-xl p-4 border border-slate-700">
                    <p class="text-[10px] text-slate-400 mb-1">المساهم</p>
                    <p class="font-bold text-sm truncate">${currentUser.name}</p>
                    <p class="text-[10px] text-brandRed mt-1">${currentUser.email}</p>
                </div>
            </div>
            <nav class="flex-1 px-4 overflow-y-auto space-y-1">${links}</nav>
    ...
    function renderHeader() {
        // تم إزالة مؤشر السوق وإضافة زر "تحديث البيانات" السريع
        ...
        <header class="h-20 bg-white/80 dark:bg-slate-900/80 backdrop-blur border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-8 sticky top-0 z-40">
            <h2 class="font-bold text-slate-800 dark:text-white">بوابة علاقات المستثمرين</h2>
            <div class="flex gap-3">
                <a href="profile.html" class="px-4 py-2 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded-full text-xs font-bold hover:bg-slate-200 transition flex items-center gap-2">
                    <i class="fa-solid fa-user-pen"></i> تحديث بياناتي
                </a>
            </div>
        </header>`;
    }
